...
from tik_core.scripts.backtest_flash import evaluate_flash_signal
...
class TestEvaluateFlashSignal:
    def test_long_success_when_price_rises(self):
        ...
        item = evaluate_flash_signal(_sig("long"), horizon_minutes=60, threshold_pct=0.1, btc_history=hist)
    ...
    def test_short_fails_when_price_rises(self):
        hist = _history_from(T0, [100.0] + [100.0] * 59 + [101.0])
        item = evaluate_flash_signal(_sig("short"), horizon_minutes=60, threshold_pct=0.1, btc_history=hist)
        assert item is not None
    ...
    def test_neutral_success_when_flat(self):
        ...
        item = evaluate_flash_signal(_sig("neutral"), horizon_minutes=60, threshold_pct=0.1, btc_history=hist)
        assert item is not None
        assert item["success"] is True
