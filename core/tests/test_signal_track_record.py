...
def _make_history(
    base_ts: datetime, prices: list[float], interval_h: float = 1.0
) -> list[tuple[int, float]]:
    """Construit une liste [(timestamp_ms, price)] à partir d'un point de départ."""
    base_ms = int(base_ts.replace(tzinfo=UTC).timestamp() * 1000)
...
class TestBadgeFor:
    ...
    def test_données_manquantes_when_p0_none(self):
        assert (
            _badge_for(available=True, p0=None, p1=95_000.0, success=None) == "données_manquantes"
        )

    def test_données_manquantes_when_p1_none(self):
        assert (
            _badge_for(available=True, p0=95_000.0, p1=None, success=None) == "données_manquantes"
        )
...
class TestComputeTrackRecordLong:
    ...
    def test_long_all_raté_when_price_falls(self):
        rows = compute_track_record(
            ...
            signal_horizon="swing",
            entity_id="BTC",
            btc_history=_btc_stable_then_down(),
            gold_history=[],
            now=_NOW,
        )
...
class TestComputeTrackRecordEdgeCases:
    ...
    def test_aware_timestamp_handled_correctly(self):
        ts_aware = _10D_AGO.replace(tzinfo=UTC)
        rows = compute_track_record(
            signal_timestamp=ts_aware,
            signal_direction="long",
            signal_horizon="swing",
            entity_id="BTC",
            btc_history=_btc_stable_then_up(),
...
class TestSignalHorizonDispatch:
    ...
    def test_legacy_alias_points_to_swing_rows(self):
        ...
        assert HORIZON_SPECS_BY_SIGNAL_HORIZON["swing"]["rows"] == TRACK_RECORD_HORIZONS
